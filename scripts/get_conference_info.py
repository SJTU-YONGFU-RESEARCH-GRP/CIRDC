...
def fetch_data(year, page, get_total_page=False, retry=10):
    ...
    headers = {
        ...
        'Referer': 'https://ieeexplore.ieee.org/browse/conferences/title',
        ...
        try:
            ...
            try:
                ...
                if not get_total_page:
                    num_of_conference = dic_obj['totalRecords']
                    json_dir = f'./tmp/json_conference_year/{year}'
...
if __name__ == "__main__":
    ...
    for year in range (1936, 2025):
        num_of_page_this_year = fetch_data(year, page=1, get_total_page=True)
        for page in range (1, num_of_page_this_year+1):
            num_of_conference = fetch_data(year, page=page, get_total_page=False)
            year_conference.append(num_of_conference)
            time.sleep(1)